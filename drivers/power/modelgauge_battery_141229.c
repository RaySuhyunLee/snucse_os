...
#define MODELGAUGE_NOT_PRESENT			0
#define MODELGAUGE_BATTERY_PRESENT		1
...
static struct modelgauge_priv *g_mg_priv;

#if defined(CONFIG_CHARGER_BQ24160)
extern int bq24160_get_batt_status(void);
#endif
...
#if defined(CONFIG_CHARGER_BQ24160)
			if ( (priv->batt_stat==0) || (bq24160_get_batt_status()==0))  {
				val->intval = FAKE_BAT_LEVEL;		/* battery not present */
				gprintk(" ###################### batt_stat=%d  intval=%d  \n\n\n", priv->batt_stat, val->intval); 
				return 0;
			}
#endif
...
int modelgauge_set_battery_status (int batt_stat)
{
	gprintk("\n\n ~~~~~~~~~~~~~~ batt_stat=%d ---> old=%d\n\n", batt_stat, g_mg_priv->batt_stat);
	if (batt_stat==1 && g_mg_priv->batt_stat==0) {
		g_mg_priv->batt_stat = batt_stat;
		modelgauge_init(g_mg_priv);
	}
	g_mg_priv->batt_stat = batt_stat;

	return 0;
}
EXPORT_SYMBOL(modelgauge_set_battery_status);
