...
#ifdef USE_HMP_BOOST
static void input_booster_set_hmp_boost(struct input_booster *data,
		enum booster_device_type device_type, bool enable)
{
	int retval = 0;
	...
	if (enable == dvfs->hmp_boosted) {
		DVFS_DEV_DBG(DBG_MISC, data->dev,
				"%s : HMP already %s for %s.\n",
				__func__, enable ? "Enabled" : "Disabled",
				booster_device_name[device_type]);
		return;
	}
...
DECLARE_DVFS_DELAYED_WORK_FUNC(CHG, TOUCH)
{
	...
	break;
	case BOOSTER_LEVEL2:
		set_qos(&dvfs->kfc_qos, PM_QOS_KFC_FREQ_MIN,
				dvfs->freqs[BOOSTER_LEVEL2].kfc_freq);
	...
	case BOOSTER_LEVEL5:
		if (dvfs->phase_excuted) {
			...
			schedule_delayed_work(&dvfs->dvfs_chg_work,
				msecs_to_jiffies(dvfs->times[BOOSTER_LEVEL5].head_time
					- dvfs->times[BOOSTER_LEVEL5].phase_time));
			dvfs->phase_excuted = true;
			DVFS_DEV_DBG(DBG_DVFS, data->dev,
					"%s : DVFS CHANGED [level %d, start time[%d]]\n",
					__func__, dvfs->level,
					dvfs->times[BOOSTER_LEVEL5].head_time -
					dvfs->times[BOOSTER_LEVEL5].phase_time);
		}
	break;
	case BOOSTER_LEVEL9:
		SET_HMP(data, BOOSTER_DEVICE_TOUCH,
				dvfs->freqs[BOOSTER_LEVEL9_CHG].hmp_boost);
		set_qos(&dvfs->kfc_qos, PM_QOS_KFC_FREQ_MIN,
				dvfs->freqs[BOOSTER_LEVEL9_CHG].kfc_freq);
		set_qos(&dvfs->int_qos, PM_QOS_DEVICE_THROUGHPUT,
...
DECLARE_DVFS_WORK_FUNC(SET, TOUCH)
{
	...
	case BOOSTER_MODE_ON:
		...
		case BOOSTER_LEVEL2:
			SET_HMP(data, BOOSTER_DEVICE_TOUCH,
					dvfs->freqs[BOOSTER_LEVEL1].hmp_boost);
			set_qos(&dvfs->kfc_qos, PM_QOS_KFC_FREQ_MIN,
					dvfs->freqs[BOOSTER_LEVEL1].kfc_freq);
			set_qos(&dvfs->int_qos, PM_QOS_DEVICE_THROUGHPUT,
		...
		case BOOSTER_LEVEL3:
			set_qos(&dvfs->kfc_qos, PM_QOS_KFC_FREQ_MIN,
					dvfs->freqs[BOOSTER_LEVEL3].kfc_freq);
			set_qos(&dvfs->int_qos, PM_QOS_DEVICE_THROUGHPUT,
			...
			remove_qos(&dvfs->cpu_qos);
			SET_HMP(data, BOOSTER_DEVICE_TOUCH,
					dvfs->freqs[BOOSTER_LEVEL3].hmp_boost);
		break;
		case BOOSTER_LEVEL4:
			set_qos(&dvfs->kfc_qos, PM_QOS_KFC_FREQ_MIN,
					dvfs->freqs[BOOSTER_LEVEL4].kfc_freq);
			set_qos(&dvfs->int_qos, PM_QOS_DEVICE_THROUGHPUT,
					dvfs->freqs[BOOSTER_LEVEL4].int_freq);
			set_qos(&dvfs->mif_qos, PM_QOS_BUS_THROUGHPUT,
					dvfs->freqs[BOOSTER_LEVEL4].mif_freq);
			remove_qos(&dvfs->cpu_qos);
			SET_HMP(data, BOOSTER_DEVICE_TOUCH,
					dvfs->freqs[BOOSTER_LEVEL4].hmp_boost);
		break;
		case BOOSTER_LEVEL5:
			SET_HMP(data, BOOSTER_DEVICE_TOUCH,
					dvfs->freqs[BOOSTER_LEVEL5].hmp_boost);
			set_qos(&dvfs->kfc_qos, PM_QOS_KFC_FREQ_MIN,
					dvfs->freqs[BOOSTER_LEVEL5].kfc_freq);
			set_qos(&dvfs->int_qos, PM_QOS_DEVICE_THROUGHPUT,
		...
		case BOOSTER_LEVEL9:
			SET_HMP(data, BOOSTER_DEVICE_TOUCH,
					dvfs->freqs[BOOSTER_LEVEL9].hmp_boost);
			set_qos(&dvfs->kfc_qos, PM_QOS_KFC_FREQ_MIN,
					dvfs->freqs[BOOSTER_LEVEL9].kfc_freq);
			set_qos(&dvfs->int_qos, PM_QOS_DEVICE_THROUGHPUT,
		...
		DVFS_DEV_DBG(DBG_DVFS, data->dev,
			"%s : DVFS ON [level %d][start time [%d]]\n",
			__func__, dvfs->level,
			dvfs->times[dvfs->level].phase_time ? :
				dvfs->times[dvfs->level].head_time);
...
DECLARE_DVFS_DELAYED_WORK_FUNC(CHG, PEN)
{
	...
	case BOOSTER_LEVEL5:
		...
		break;
	case BOOSTER_LEVEL9:
		SET_HMP(data, BOOSTER_DEVICE_PEN,
				dvfs->freqs[BOOSTER_LEVEL9_CHG].hmp_boost);
		set_qos(&dvfs->kfc_qos, PM_QOS_KFC_FREQ_MIN,
				dvfs->freqs[BOOSTER_LEVEL9_CHG].kfc_freq);
		set_qos(&dvfs->int_qos, PM_QOS_DEVICE_THROUGHPUT,
...
static void input_booster_lookup_freqs(struct booster_dvfs *dvfs)
{
	...
	for (i = 0; i < BOOSTER_LEVEL_MAX; i++) {
		...
		dev_info(data->dev,
			"%s: %s's LEVEL%d cpu %d kfc %d mif %d int %d hmp %d\n",
			__func__, dvfs->name, i, dvfs->freqs[i].cpu_freq,
			dvfs->freqs[i].kfc_freq, dvfs->freqs[i].mif_freq,
			dvfs->freqs[i].int_freq, dvfs->freqs[i].hmp_boost);
	}
}
...
static void input_booster_lookup_times(struct booster_dvfs *dvfs)
{
	...
	for (i = 0; i < BOOSTER_LEVEL_MAX; i++) {
		...
		dev_info(data->dev,
			"%s: %s's LEVEL%d head %d tail %d phase %d\n",
...
static DEVICE_ATTR(level, S_IRUGO | S_IWUSR, input_booster_get_dvfs_level,
		input_booster_set_dvfs_level);
static DEVICE_ATTR(freq, S_IRUGO | S_IWUSR, input_booster_get_dvfs_freq,
		input_booster_set_dvfs_freq);
static DEVICE_ATTR(time, S_IRUGO | S_IWUSR, input_booster_get_dvfs_time,
		input_booster_set_dvfs_time);
...
static void input_booster_free_dvfs_sysfs(struct input_booster *data,
		struct booster_dvfs *dvfs)
{
...
static void input_booster_init_dvfs_sysfs(struct input_booster *data,
		struct booster_dvfs *dvfs)
{
	int ret;
...
static int input_booster_init_dvfs(struct input_booster *data,
		struct booster_device *device)
{
	...
	if (data->dvfses[device_type] != NULL &&
			data->dvfses[device_type]->initialized) {
		dev_info(data->dev,
			"%s: %s is already initialized [device = %s]\n",
			__func__, booster_device_name[device_type],
			device->desc);
		goto out;
	}
	...
	memcpy(dvfs->freqs, device->freq_table,
			sizeof(struct dvfs_freq) * BOOSTER_LEVEL_MAX);
	memcpy(dvfs->times, device->time_table,
			sizeof(struct dvfs_time) * BOOSTER_LEVEL_MAX);
...
static int input_booster_parse_dt(struct device *dev)
{
	...
	pdata->devices = devm_kzalloc(dev,
			sizeof(struct booster_device) * pdata->ndevices,
			GFP_KERNEL);
	if (!pdata->devices) {
	...
	for_each_child_of_node(np, cnp) {
		...
		booster_device->desc = of_get_property(cnp,
				"input_booster,label", NULL);
		...
		if (plevels && num_level) {
			...
			dev_err(dev,
				"Failed to calculate number of frequency.\n");
			retval = -EINVAL;
			goto err_out;
		}
		...
		dev_info(dev, "device_type:%d, label :%s, type: 0x%02x, num_level[%d]\n",
			device_type, booster_device->desc,
			booster_device->type, num_level);
		for (i = 0; i < BOOSTER_LEVEL_MAX; i++) {
...
static int input_booster_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		pdata = devm_kzalloc(&pdev->dev,
			sizeof(struct input_booster_platform_data),
			GFP_KERNEL);
	...
	memcpy(data->devices, pdata->devices,
			sizeof(struct booster_device) * pdata->ndevices);

#ifdef BOOSTER_DEBUG
	for (i = 0; i < pdata->ndevices; i++) {
		dev_info(&pdev->dev,
			"%s: data->devices[%d] : desc=%s, type=0x%02X\n",
			__func__, i, data->devices[i].desc,
			data->devices[i].type);
	}
#endif
