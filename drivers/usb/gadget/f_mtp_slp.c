...
static struct usb_ss_ep_comp_descriptor mtpg_intr_comp_desc = {
	...
	/* .bMaxBurst		= 0, */
	/* .bmAttributes	= 0, */
	.wBytesPerInterval	= __constant_cpu_to_le16(28),
};

static struct usb_endpoint_descriptor mtpg_intr_desc = {
	.bLength                = USB_DT_ENDPOINT_SIZE,
	.bDescriptorType        = USB_DT_ENDPOINT,
	.bEndpointAddress       = USB_DIR_IN,
	.bmAttributes           = USB_ENDPOINT_XFER_INT,
	.wMaxPacketSize         = __constant_cpu_to_le16(28),
	.bInterval              = 6,
};
...
static struct usb_descriptor_header *ss_mtpg_desc[] = {
	...
	(struct usb_descriptor_header *) &mtpg_superspeed_bulk_comp_desc,
	(struct usb_descriptor_header *) &mtpg_intr_desc,
	(struct usb_descriptor_header *) &mtpg_intr_comp_desc,
	NULL,
};
...
static long mtpg_ioctl(struct file *fd, unsigned int code, unsigned long arg)
{
	...
	if (code == SET_MTP_USER_PID) {
		...
		/* Previously at tx-case, we use variable size in udc-max-tx-pkt-size
		 ...
		 *
		 * Now because of PLM :- P150707-05191, in mtpg_write() when
		 * we try to allocate a tx buff of size 64KB contiguous memory
		 * kmalloc fails as below :-
		 * [mtpg_write]malloc retry stop:bytes(65536), try_cnt(1).
		 * This may be because the system memory is already fragmented
		 * and 64k is of order 4 - big size
		 */

		size_info.txPktSize = dev->bulkBufSize;
	...
	cdev = dev->mtp_func->function.config->cdev;
	if (!cdev) {
		pr_info("mtpg_ioctl fail, config not yet enabled for MTP\n");
		mutex_unlock(&mtp_lock);
		return -ENODEV;
	}
	req = cdev->req;
			      ...
			      struct usb_function *f)
{
	...
	if (gadget_is_superspeed(cdev->gadget)) {
		...
		mtpg_superspeed_out_desc.bEndpointAddress =
			fs_mtpg_out_desc.bEndpointAddress;
		mtpg_intr_desc.bEndpointAddress =
			int_fs_notify_desc.bEndpointAddress;
